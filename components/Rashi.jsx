...
const Rashi = () => {
  ...
  useEffect(() => {
    filterData();
  }, [selectedGender, selectedCulture, selectedRashi, currentPage]);
  ...
  const filterData = () => {
    ...
    if (selectedGender) {
      filtered = filtered.filter(
        (entry) => entry.Gender.toLowerCase() === selectedGender.toLowerCase()
      );
    }

    if (selectedCulture) {
      filtered = filtered.filter(
        (entry) => entry.Culture.toLowerCase() === selectedCulture.toLowerCase()
      );
    }

    if (selectedRashi) {
      // Define the rashi initials mapping
      const rashiInitials = {
        Dhanu: ["BH", "F", "DH"],
        Kanya: ["P", "THA"],
        Karka: ["DD", "H"],
      ...
      // Filter based on selected rashi
      filtered = filtered.filter(
        (entry) =>
          entry &&
          entry.Name &&
  ...
  return (
    <div className="w-11/12 m-auto pt-28">
      <div className="flex justify-between mb-5">
        <GenderSearch genderSelected={setSelectedGender} />
        <select
        ...
        >
          ...
          <option value="Dhanu">Dhanu/Sagittarius (BH,F,DH)</option>
          <option value="Kanya">Kanya/Virgo (P,THA)</option>
          <option value="Karka">Karka/Cancer (DD,H)</option>
          <option value="Kumbha">Kumbha/Aquarius (G,S,Sh)</option>
          <option value="Makar">Makar/Capricorn (KH,J)</option>
          <option value="Meena">Meena/Pisces (D,CH,Z,TH)</option>
          <option value="Mesha">Mesha/Aries (A,L,E,I,O)</option>
          <option value="Mithun">Mithun/Gemini (K,CHH,GH,Q,C)</option>
          <option value="Simha">Simha/Leo (M,TT)</option>
          <option value="Tula">Tula/Libra (R,T)</option>
          <option value="Vrishabha">Vrishabha/Taurus (B,V,U,W)</option>
          <option value="Vruschika">Vruschika/Scorpio (N,Y)</option>
        </select>
      </div>
            ...
            .map((entry, index) => (
              ...
              >
                <td className="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                  <Link href={`/indian/baby-name/${entry.Name.toLowerCase()}`}>
                    {entry.Name}
                  </Link>
                </td>
                <td className="px-4 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
